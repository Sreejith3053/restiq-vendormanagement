...
async function check() {
    ...
    let thymeId = null;
    snap.docs.forEach(d => {
        if ((d.data().name || '').toLowerCase().includes('thyme')) thymeId = d.id;
    });

    if (thymeId) {
        const items = await db.collection(`vendors/${thymeId}/items`).get();
        items.docs.forEach(d => console.log(d.id, '->', d.data().name, d.data().price));
    }
}
check();
