...
// Import the logic directly from the functions module we created
...
async function runPipeline() {
    console.log("=== Running local mock test of Forecast Engine ===");
    try {
        await runDeterministicForecast(db);
        await aggregateForecasts(db);
        await generateVendorRollups(db);
        await checkForecastAccuracy(db); // Generate dummy logs for UI testing
        console.log("✅ Pipeline mock run successfully over recently seeded data.");
    } catch (err) {
