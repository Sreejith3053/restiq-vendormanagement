 ...
 * Clickable — clicking a card filters the table to the matching type.
 */
...
const CARDS = [
    ...
    { key: 'unmappedVendorItems',  label: 'Unmapped Items',     emoji: '🔍', color: '#38bdf8', bg: 'rgba(56,189,248,0.07)',    border: 'rgba(56,189,248,0.22)' },
    { key: 'held',                 label: 'Held',               emoji: '🕐', color: '#fbbf24', bg: 'rgba(251,191,36,0.07)',    border: 'rgba(251,191,36,0.22)' },
    { key: 'approvedToday',        label: 'Approved Today',     emoji: '✅', color: '#10b981', bg: 'rgba(16,185,129,0.07)',    border: 'rgba(16,185,129,0.22)' },
    { key: 'rejectedToday',        label: 'Rejected Today',     emoji: '❌', color: '#f43f5e', bg: 'rgba(244,63,94,0.07)',     border: 'rgba(244,63,94,0.22)' },
];

export default function ReviewSummaryCards({ summary = {}, loading = false, onCardClick, activeCard }) {
                ...
                return (
                    <div
                        ...
                        style={{
                            ...
                            background: isActive ? `${card.color}18` : card.bg,
                            border: isActive ? `2px solid ${card.color}` : `1px solid ${card.border}`,
                            textAlign: 'center',
                            opacity: loading ? 0.5 : 1,
                            transition: 'all 0.2s',
                            cursor: onCardClick ? 'pointer' : 'default',
                            transform: isHovered && onCardClick ? 'translateY(-2px)' : 'none',
                            boxShadow: isHovered && onCardClick ? `0 4px 16px ${card.color}15` : 'none',
                        }}
                    >
