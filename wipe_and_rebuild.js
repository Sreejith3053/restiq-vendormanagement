const admin = require('firebase-admin');

process.env.GOOGLE_GENAI_API_KEY = "mock-key";
process.env.GEMINI_API_KEY = "mock-key";

const rmsServiceAccount = {
    type: "service_account",
    project_id: "orumarmsprod",
    private_key_id: "8e8360ad7717da54dd456125378a9feb1ddf5854",
    private_key: process.env.RMS_PRIVATE_KEY || "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    client_email: "firebase-adminsdk-fbsvc@orumarmsprod.iam.gserviceaccount.com",
    client_id: "100723697611119927274",
    auth_uri: "https://accounts.google.com/o/oauth2/auth",
    token_uri: "https://oauth2.googleapis.com/token",
    auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
    client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40orumarmsprod.iam.gserviceaccount.com",
    universe_domain: "googleapis.com"
};

admin.initializeApp({
    credential: admin.credential.cert(rmsServiceAccount),
    projectId: 'orumarmsprod'
});
const db = admin.firestore();

// Import the logic directly from the functions module we created (now in same dir)
const { runDeterministicForecast, aggregateForecasts, generateVendorRollups } = require('./functions/forecastEngine');
const { checkForecastAccuracy } = require('./functions/forecastAccuracy');

async function wipeAndRebuild() {
    console.log("=== Wiping Data ===");
    const collections = ['restaurantItemForecasts', 'aggregateItemForecasts', 'vendorPlanningForecasts', 'forecastAccuracyLogs'];
    for (const col of collections) {
        const snap = await db.collection(col).get();
        const batch = db.batch();
        snap.forEach(doc => {
            batch.delete(doc.ref);
        });
        await batch.commit();
        console.log(`Wiped ${snap.size} docs from ${col}`);
    }

    console.log("=== Building ===");
    await runDeterministicForecast(db);
    await aggregateForecasts(db);
    await generateVendorRollups(db);
    await checkForecastAccuracy(db);

    console.log("=== Built! Listing resulting keys ===");
    for (const col of collections) {
        const snap = await db.collection(col).get();
        console.log(`- ${col}`);
        snap.forEach(d => console.log(`  ${d.id} -> ${d.data().weekStart}`));
    }
    process.exit(0);
}

wipeAndRebuild();
